    ...
    where TA : class, IAggregateRoot<TA, TKey>
{
    ...
    public async Task PersistAsync(TA aggregateRoot, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var prevAggregateEvents = _client.QueryAsync<EventData>(ed => ed.PartitionKey == aggregateRoot.Id.ToString() && ed.AggregateVersion >= expectedVersion,
    ...
    public async Task<TA> RehydrateAsync(TKey key, CancellationToken cancellationToken = default)
    {
        ...
        {
            var @event = _eventSerializer.Deserialize<TA, TKey>(row.EventType, row.Data);
            events.Add(@event);
        }
